...
How it works
------------
Each mutation is a literal source-text substitution into `snn/kernels.py` or
`snn/surrogate.py`, applied one at a time, after which the real R10 gate is run
unmodified. A mutation is CAUGHT if the gate fails and ESCAPED if it passes. An
escaped mutation is a hole in the gate and is reported as one.
...
# Each entry: (id, description, file, find, replace).
...
def run_gate(timeout_s: int) -> tuple[bool, str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan = [m for m in MUTATIONS if not wanted or m[0] in wanted]
    ...
    for mid, desc, path, find, _repl in plan:
    ...
    print("baseline: the gate must PASS on unmutated source", flush=True)
    t0 = time.perf_counter()
    base_pass, base_tail = run_gate(args.timeout)
    base_s = time.perf_counter() - t0
    print(f"  {'PASS' if base_pass else 'FAIL'} in {base_s:.0f}s")
    if not base_pass:
        raise SystemExit(f"the gate does not pass on clean source:\n{base_tail}")

    results: dict = {
        "audit": "08_mutation_campaign",
        "gate": GATE,
        "baseline_pass": base_pass,
        "baseline_seconds": round(base_s, 1),
        "n_mutations": len(plan),
        "mutations": {},
    }
    escaped: list[str] = []

    for i, (mid, desc, path, find, repl) in enumerate(plan, 1):
        ...
        try:
            ...
            t0 = time.perf_counter()
            passed, tail = run_gate(args.timeout)
